...
use bigint::big_digit::{adc, sbb, mul_with_carry, mac_with_carry, div_wide};
...
fn cmp_slice(a: &[BigDigit], b: &[BigDigit]) -> Ordering {
    ...
    for (&self_i, &other_i) in a.iter().rev().zip(b.iter().rev()) {
...
impl Shl<usize> for BigUint {
    ...
    fn shl(mut self, rhs: usize) -> BigUint {
        ...
        self.data.extend(repeat(0).take(n_unit));
        ...
        loop {
            let mut w = self.data[src] << n_bits;
...
fn add2(a: &mut [BigDigit], b: &[BigDigit]) {
    ...
    assert!(b_iter.next() == None && carry == 0);
}
...
impl<'a> Add<&'a [BigDigit]> for BigUint {
    ...
    fn add(mut self, other: &[BigDigit]) -> BigUint {
        let new_len = cmp::max(self.data.len(), other.len()) + 1;
...
fn sub_sign(a: &[BigDigit], b: &[BigDigit]) -> BigInt {
    fn cmp(a: &[BigDigit], b: &[BigDigit]) -> Ordering {
        /*
    ...
    match cmp(a, b) {
        Greater => BigInt::from_biguint(Plus, BigUint::from_slice(a) - b),
        Less    => BigInt::from_biguint(Minus, BigUint::from_slice(b) - a),
        _       => Zero::zero(),
    }
}
...
impl Integer for BigUint {
    ...
    fn div_mod_floor(&self, other: &BigUint) -> (BigUint, BigUint) {
        ...
        let (q, m) = div_mod_floor_inner(self << shift, other << shift);
        return (q, m >> shift);


        fn div_mod_floor_inner(mut a: BigUint, b: BigUint) -> (BigUint, BigUint) {
            /*
            ...
            /* We reuse the same temporary to avoid hitting the allocator in our inner loop: */
